...
def Create_connection_string():
     ...
     if MavVars.ConnectTo==2: # pixhawk waiting for connection
          ...
          MavVars.connection_string = "tcp:"+MavVars.vehIPAddr+":"+MavVars.vehIPport
          print("ConnectTo= 2 tcp:+MavVars.vehIPAddr+:+MavVars.vehIPport  MavVars.connection_string=", MavVars.connection_string)
...
def connectVehicle():
    ...
    if MavVars.ConnectTo==2: # pixhawk waiting for connection
        ...
        MavVars.connection_string = Create_connection_string()
        #
        MavVars.vehicle = mavutil.mavlink_connection(MavVars.connection_string , source_system=1, wait_ready=True,  ) 
        ...
        if MavVars.vehicle != None: # if connection made print message
            ...
            MavVars.connection_start_time = datetime.now() 
            mainWin.ControlWin.clickConnectButton()
            print("Connection made to SITL vehicle, info: (connection_string %s targ.system %u targ.component %u src.sys %u src.comp %u)" % (MavVars.connection_string, MavVars.vehicle.target_system, MavVars.vehicle.target_component, MavVars.vehicle.source_system, MavVars.vehicle.source_component))
    ...
    if MavVars.ConnectTo==3:  # connect to mavlink autopilot via serial port
        ...
        MavVars.connection_string = Create_connection_string()
        print("MavVars.vehSERBaud=", MavVars.vehSERBaud , " MavVars.connection_string=", MavVars.connection_string)
        MavVars.vehicle = mavutil.mavlink_connection(MavVars.connection_string , baud=int(str(MavVars.vehSERBaud)) ) #, baud=115200, source_system=125) # , source_system=125 # doesnt include listeners
        if MavVars.vehicle != None: # if connection made print message
            MavVars.veh_connected = True
            MavVars.connection_start_time = datetime.now() 
            mainWin.ControlWin.clickConnectButton()
    ...
    if f:
        ...
        for child in root:
            ...
            for field in child:
                ...
                for child in child:
                    if printXMLmsgsParse==True: print(child.tag, child.attrib) #  'enum'
                    msgDict=child.attrib 
                    msgDicttag = child.tag
                    if printXMLmsgsParse==True: print(" ***msgDict.tag ",msgDicttag, " ***msgDict ",msgDict)
                    ...
                    if child.tag=="enum":       #print("enumdetected")
                        ...
                        for child1 in child:    #.iter('field'):
                            if child1.tag != 'description': # dont print description tag ?
                                ...
                                if printXMLmsgsParse==True: print("    child enum:", child1.tag, child1.attrib)                         
                        ...
                        tmp2 = tmp.replace('}', '') 
                        if printXMLmsgsParse==True: print ("$$$$ str(msgDict) ",str(msgDict), tmp) #$$$$ str(msgDict)  {'name': 'FAILURE_TYPE'}
                        if printXMLmsgsParse==True: print("$$$$$ ", tmp2)   # name
                        #enum_str = " {'" + tmp2 + "'}," + 
                        enum_str = " {'" + enum_str + "'}" 
                        MavVars.MAVLinkEnumsDict[tmp2] = enum_str 
                        if printXMLmsgsParse==True: print("$$$$$$ tmp2", tmp2, " MavVars.MAVLinkEnumsDict[tmp2] ", MavVars.MAVLinkEnumsDict[tmp2],"\n\nMavVars.MAVLinkEnumsDict{}:")                   
                    ...
                    if child.tag=="message":    #print("messagedetected")
                        ...
                        for child1 in child:    #.iter('field'):
                            if child1.tag != 'description': # dont print description tag
                                ...
                                for child2 in child1:    #.iter('field'):
                                    if printXMLmsgsParse==True: print("    child2 message:", child2.tag, child2.attrib)

                            else: #  child1.tag == description
                               fields_flds_enum_str = fields_flds_enum_str + "{'" + str(child1.tag) + "':"  +  "'" + str(child1.text) + "'}"
                               #if printXMLmsgsParse==True: print( "###endmsg ", keyval, "message fields enums_flds_enum_str ", fields_flds_enum_str , " endmsg###")
                               if printXMLmsgsParse==True: print("    child1 message: ", child1.tag, child1.text)
...
def CheckMAVmsgForMatch():
    ...
    #
    if msg!= None: 
        # send attitude data from pixhawk to external server over tcp or udp  , meant for G5 sim
        ...
        if RcvLOGGINGALL==1:  # print to xterm command line for now
            if RcvON  == 1: print(date_time, 'MAV: ', msg) 
        ...
        if RcvMATCHON == 1:
            ...
            for key in MavVars.MAVmsgsToWatchDict :       
                # print(date_time, "TRYING MATCH ? FOR MavVars.MAVmsgsToWatchDict[key] , msg: ", MavVars.MAVmsgsToWatchDict[key], "  ", msg )
...
MavVars.ConnectTo = 1  # connectTo an SITL or autopilot; 1=startup and use internal sitl, takes longer, 2=connect vehicle using ip addr of sitl , 3=connect using serial port
...
if __name__ == '__main__':
    ...
    QTimer.singleShot(1000, mavmsgPoll) # always receiving , afterstartup delays RetryRxMs in mavmsgPoll()  starts polling for mavlink message using pymavlink dronekit and sitl 3.3
    # 
