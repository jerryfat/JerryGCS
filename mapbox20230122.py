...
# Start SITL if no connection , eg string empty
if not connection_string:
    import dronekit_sitl
    start_lat =  40.17904123640337 #, -74.6825621480185# 41.833474
    start_lon = -74.6825621480185 # -87.626819
    sitl = dronekit_sitl.start_default(start_lat, start_lon)   #sitl = dronekit_sitl.start_default()
    connection_string = sitl.connection_string()
    print("sitl.connection_string: ", connection_string)
...
class Drone(object):
    ...
    def launch(self):
        ...
        self._log("self.vehicle.location.global_frame.lat:")
        self._log(self.vehicle.location.global_frame.lat)      
        self._log("self.vehicle.location.global_frame.lon:") 
        self._log(self.vehicle.location.global_frame.lon)    
...
class Templates:
    ...
    def get_options(self):
        return {'width': 670,
                ...
                'zoom': 13,
                'format': 'png',
                'access_token': 'pk.eyJ1IjoiamVycnlmYXQiLCJhIjoiY2tjczFwZnp0MDJuaDJ6cWpkZHF5YjM1cCJ9.8pwSnCZUuvsm8Tzm_LYt4g',
                ...
                # 'access_token': 'pk.eyJ1Ijoia2V2aW4zZHIiLCJhIjoiY2lrOGoxN2s2MDJzYnR6a3drbTYwdGxmMiJ9.bv5u7QgmcJd6dZfLDGoykw','mapid':           #'kevin3dr.n56ffjoo',
                #works for jerry
                # 'access_token': 'pk.eyJ1IjoiamVycnlmYXQiLCJhIjoiY2tjczFwZnp0MDJuaDJ6cWpkZHF5YjM1cCJ9.8pwSnCZUuvsm8Tzm_LYt4g',
                # 'mapid': 'mapbox.satellite',
                #mapbox.satellite
...
class DroneDelivery(object):
    ...
    def track(self, lat=None, lon=None):
        ...
        #self._log("TRACK() Goto: {0}, {1}".format(location, self.altitude))
